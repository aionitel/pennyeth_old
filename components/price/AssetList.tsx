...
const AssetList = () => {
  const allAssetData: AssetProps[] = useRecoilValue(allAssetsAtom);

  return (
    <div>
      {
        allAssetData.map(item => (
          <div key='all-assets' className='flex'>
          <AssetListItem
            key=''
            name={item.name} 
            ticker={item.ticker}
...
const AssetListItem: React.FC<AssetProps> = ({ name, ticker, image, price, dailyChange, volume, marketCap }) => {
  const logoSize = 40;
